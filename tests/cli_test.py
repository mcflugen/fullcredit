...
class TestContributors:
    ...
    def test_with_sep(self, capsys, sep):
        with patch(
            "fullcredit._cli.collect_git_contributors",
            return_value={
                ("John Cleese", "john@example.com"),
    ...
    def test_output_is_sorted(self, capsys):
        with patch(
            "fullcredit._cli.collect_git_contributors",
            return_value={
...
class TestAuthors:
    ...
    def test_deduplicates(self, capsys):
        with patch(
            "sys.stdin",
            io.StringIO(
                "Graham Chapman\0graham@example.com\n"
    ...
    def test_invalid_identity_raises(self, capsys):
        with (
            patch("sys.stdin", io.StringIO("not-null-delimited\n")),
            pytest.raises(ValueError, match="invalid identity"),
        ):
            main(["authors"])
...
class TestInit:
    def test_produces_toml_with_author_table(self, capsys):
        with patch(
            "fullcredit._cli.collect_git_contributors",
    ...
    def test_output_is_sorted_by_name(self, capsys):
        with patch(
            "fullcredit._cli.collect_git_contributors",
            return_value={
                ("Terry Gilliam", "terry@example.com"),
...
class TestMerge:
    def test_merges_stdin_with_file(self, capsys, tmp_path):
        db = tmp_path / "authors.toml"
        db.write_bytes(JOHN_TOML)
        with patch("sys.stdin", _binary_stdin(GRAHAM_TOML)):
    ...
    def test_merges_overlapping_records(self, capsys, tmp_path):
        extra = b"""\
[[author]]
name = "G. Chapman"
email = "graham@example.com"
...
"""
        ...
        with patch("sys.stdin", _binary_stdin(extra)):
            assert main(["merge", str(db)]) == 0
        out = capsys.readouterr().out
        assert out.count("[[author]]") == 1

    def test_stdin_only(self, capsys):
        with patch("sys.stdin", _binary_stdin(GRAHAM_JOHN_TOML)):
            assert main(["merge"]) == 0
        out = capsys.readouterr().out
        assert "Graham Chapman" in out
        assert "John Cleese" in out

    def test_file_arg_takes_priority_over_stdin(self, capsys, tmp_path):
...
"""
        db = tmp_path / "authors.toml"
        db.write_bytes(GRAHAM_TOML)
        with patch("sys.stdin", _binary_stdin(g_chapman_toml)):
            assert main(["merge", str(db)]) == 0
        data = tomllib.loads(capsys.readouterr().out)
        assert data["author"][0]["name"] == "Graham Chapman"

    def test_last_file_arg_takes_priority(self, capsys, tmp_path):
...
"""
        ...
        with patch("sys.stdin", _binary_stdin(JOHN_TOML)):
...
class TestSort:
    def test_sort_by_name(self, capsys):
        with patch("sys.stdin", _binary_stdin(SORT_TOML)):
            assert main(["sort", "name"]) == 0
        names = _parse_names_from_toml_output(capsys.readouterr().out)
        assert names == sorted(names, key=str.casefold)

    def test_sort_by_last_name(self, capsys):
        with patch("sys.stdin", _binary_stdin(SORT_TOML)):
    ...
    def test_sort_reverse(self, capsys):
        with patch("sys.stdin", _binary_stdin(SORT_TOML)):
    ...
    def test_sort_by_last_name_single_word(self, capsys):
...
alternate_emails = []
"""
        with patch("sys.stdin", _binary_stdin(single_word_toml)):
            assert main(["sort", "last-name"]) == 0
        import tomllib
...
alternate_emails = []
"""
        with patch("sys.stdin", _binary_stdin(GRAHAM_TOML + b"\n" + bot_toml)):
            assert main(["build", r"--exclude=\[bot\]"]) == 0
        out = capsys.readouterr().out
        assert "Graham Chapman" in out
        assert "spam" not in out

    def test_one_line_per_author(self, capsys):
        with patch("sys.stdin", _binary_stdin(GRAHAM_JOHN_TOML)):
            assert main(["build"]) == 0
...
class TestMailmap:
    def test_generates_entries_for_aliases(self, capsys):
        with patch("sys.stdin", _binary_stdin(MAILMAP_TOML)):
    ...
    def test_no_aliases_produces_no_entries(self, capsys):
        with patch("sys.stdin", _binary_stdin(GRAHAM_TOML)):
            assert main(["mailmap"]) == 0
        assert capsys.readouterr().out.strip() == ""

    def test_invalid_toml_returns_error(self, capsys):
        with patch("sys.stdin", _binary_stdin(b"[not_an_author_table]\nfoo = 'bar'\n")):
